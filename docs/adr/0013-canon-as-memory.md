...
> Still partial: substantive agent-authored distillation, semantic parent
> reconciliation, and the ADR 0051/0047 durable summary inputs await the real runtime.
...
**The canonical documents are the long-term knowledge memory; we do not archive raw
history as canon.** Durable ticket-attached decision records are operational memory for
active or recently blocked work (ADR 0051). Their job is to make work resumable and
auditable until the relevant result is ratified. Once a durable lesson changes project
intent, it belongs in canon. The durable footprint of a completed subtree is therefore a
diff to canon plus compact operational summaries, not a pile of transcripts.
...
- **Decision history as canon signal.** Ticket-attached requests and responses are not
  automatically canon, but they are input to context assembly and canon distillation.
  Repeated ambiguities, rejected proposals, and human clarifications should drive SOP,
  policy, architecture, and contract updates.
