...
real event streaming, real WIP commits — is Phase 6 (E-0006). Phase 2 still needs
...
- **`runtime.Interface` + records-only stub.** A minimal runtime interface is
  ...
  exact seam the Codex adapter fills in Phase 6 (E-0006). The stub is justified:
...
- **Checkpoints as records, squash as semantics.** M2 stores checkpoint records
  ...
  and resume-from-checkpoint (T-0904) are Phase 6, because they require the real
...
artifact is the git ref, which the Phase 6 runtime creates. `recovery.md`'s "mark stale runs interrupted / expire
leases" is fully implementable now; "resume from last checkpoint" is explicitly
Phase 6. The records-vs-execution split keeps the durable-state boundary
