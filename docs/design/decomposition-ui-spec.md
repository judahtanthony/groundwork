...
- filters by kind, status, agent, and "leaf only" (the primary unit operators act on).
...
- right-rail fields gain `kind`, `node_type`, and `depends_on`.
...
- add an **SOPs** surface: task-type SOPs and context under `.groundwork/sops/<task-type>/`, with the maturity signals (SOP + context + validations) that justify elevating an action's autonomy. Elevation remains an explicit human act; learning may only *suggest* it (reuse the existing suggestion queue).
